...
func main() {
	...
	// --- CÓDIGO NUEVO: Publicar un mensaje de ejemplo al inicio ---
	fmt.Println("Publicando mensaje de prueba...")
	exampleMessage := `{"lat": -34.6037, "lng": -58.3816, "city": "buenos_aires"}`
	result := topicObj.Publish(ctx, &pubsub.Message{
		Data: []byte(exampleMessage),
	})
	// Esperamos a que el mensaje se publique
	id, err := result.Get(ctx)
	if err != nil {
		log.Printf("Error al publicar el mensaje de ejemplo: %v", err)
	} else {
		fmt.Printf("Mensaje de ejemplo publicado con ID: %s\n", id)
	}
